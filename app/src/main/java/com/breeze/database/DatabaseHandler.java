...
import com.breeze.datatypes.BrzProfile;
...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    private static final String BRZPROFILE_TABLE_NAME = "BrzProfile";
    ...
    private static final String INIT_BRZPROFILE_TABLE = "CREATE TABLE IF NOT EXISTS BrzProfile ('id' TEXT PRIMARY KEY, " +
            ...
            "'profilepic' STRING," +
            "FOREIGN KEY (nodeId) REFERENCES BrzNode(id))";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(INIT_BRZPROFILE_TABLE);
    ...
    public void addProfile(BrzProfile profile){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues vals = new ContentValues();
        vals.put("id", profile.getId());
        vals.put("nodeId", profile.getNodeId());
        vals.put("name", profile.getName());
        vals.put("alias", profile.getAlias());
        vals.put("signature", profile.getSignature());
        vals.put("blocked", profile.isBlocked());
        vals.put("friend", profile.isFriend());
        vals.put("profilepic", profile.getProfilePicture());
        db.insert(BRZPROFILE_TABLE_NAME, null, vals);
        db.close();
    }

    public void deleteProfile(@NonNull String id){
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete(BRZPROFILE_TABLE_NAME, "id = ?", new String[]{String.valueOf(id)});
        db.close();
    }

    public BrzProfile getProfile(@NonNull String id){
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c = db.rawQuery("SELECT * FROM " + BRZPROFILE_TABLE_NAME + " WHERE id = ?;", new String[]{id});
        if (c == null) {
            db.close();
            return null;
        } else if (c.getCount() < 1) {
        ...
        BrzProfile profile = new BrzProfile();
        c.moveToFirst();
        profile.setId(c.getString(c.getColumnIndex("id")));
        profile.setName(c.getString(c.getColumnIndex("name")));
        profile.setAlias(c.getString(c.getColumnIndex("alias")));
        profile.setSignature(c.getString(c.getColumnIndex("signature")));
        profile.setFriend(c.getString(c.getColumnIndex("friend")).equals("1"));
        profile.setBlocked(c.getString(c.getColumnIndex("blocked")).equals("1"));
        profile.setProfilePicture(c.getString(c.getColumnIndex("profilepicture")));
        profile.setNodeId(c.getInt(c.getColumnIndex("nodeId")));
        c.close();
        db.close();
        return profile;
    }

    public List<BrzProfile> getFriends() {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c = db.rawQuery("SELECT * FROM " + BRZPROFILE_TABLE_NAME + " WHERE 'friends' = ?;", new String[]{"1"});
        if (c == null) {
            db.close();
            return null;
        } else if (c.getCount() < 1) {
        ...
        ArrayList<BrzProfile> list = new ArrayList<>();
        if (c.moveToFirst()) {
            while (!c.isAfterLast()) {
                BrzProfile profile = new BrzProfile();
                profile.setId(c.getString(c.getColumnIndex("id")));
    ...
    public List<BrzProfile> getBlocked() {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c = db.rawQuery("SELECT * FROM " + BRZPROFILE_TABLE_NAME + " WHERE 'blocked' = ?;", new String[]{"1"});
        if (c == null) {
            db.close();
            return null;
        } else if (c.getCount() < 1) {
    ...
    public BrzProfile getByAlias(@NonNull String alias){
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c = db.rawQuery("SELECT * FROM " + BRZPROFILE_TABLE_NAME + " WHERE alias = ?;", new String[]{alias});
        if (c == null) {
            db.close();
            return null;
        } else if (c.getCount() < 1) {
